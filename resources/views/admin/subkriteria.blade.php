            ...
            <table class="w-full text-sm text-left row-border text-gray-500 divide-y divide-gray-300" id="myTable">
                ...
                <tbody>
                    ...
                    <tr class="bg-white border-b text-center">
                        ...
                        <td class="px-6 py-4">
                            <a href="{{ url('/dashboard/subkriteria/edit/' . $sk->sub_kriteria_id) }}">
                            ...
                            <button type="button" data-modal-target="popup-modal-{{ $sk->sub_kriteria_id }}"
                                data-modal-toggle="popup-modal-{{ $sk->sub_kriteria_id }}"
                                class=" text-white tracking-wide focus:outline-none focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2 bg-red-600 hover:bg-red-700 focus:ring-red-900">Delete</button>

                            <div id="popup-modal-{{ $sk->sub_kriteria_id }}" tabindex="-1"
                                class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
                                <div class="relative w-full max-w-md max-h-full">
                                    <div class="relative bg-white rounded-lg shadow">
                                        <button type="button"
                                            class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                                            data-modal-hide="popup-modal-{{ $sk->sub_kriteria_id }}">
                                        ...
                                        <form action="{{ route('/dashboard/subkriteria/delete') }}" method="POST"
                                            ...
                                            <button data-modal-hide="popup-modal-{{ $sk->sub_kriteria_id }}" type="submit"
                                                class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center mr-2">
                                                Ya, Saya yakin
                                            </button>
                                            <button data-modal-hide="popup-modal-{{ $sk->sub_kriteria_id }}" type="button"
